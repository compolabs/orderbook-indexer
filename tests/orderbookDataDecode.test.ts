...
describe("Envio indexer data encode test", () => {
  ...
  it("should decode data", async () => {
    ...
    const receiptsResult = await fetchReceiptsFromEnvio(blockNumber, blockNumber + 50000, [
      contractId,
    ]);
    const orderbookAbi = OrderbookAbi__factory.connect(contractId, wallet);
    if (receiptsResult === null) return;
    console.log(
      decodeOrderbookReceipts(receiptsResult.receipts, orderbookAbi!).filter(
        ({ order_id }: any) =>
          order_id == "0x6deec5429eb0e1eba4a0e9cac06cd210412741e43842ad8ff42b2dcb155553e7"
      )
    );
  }, 60_000);
});
